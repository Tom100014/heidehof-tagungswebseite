...
GRANT SELECT ON public.phone_agent_calls TO authenticated;
GRANT ALL ON public.phone_agent_calls TO service_role;
...
DROP POLICY IF EXISTS "Admins can read phone agent calls" ON public.phone_agent_calls;
CREATE POLICY "Admins can read phone agent calls"
  ON public.phone_agent_calls FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Service role manages phone agent calls" ON public.phone_agent_calls;
CREATE POLICY "Service role manages phone agent calls"
  ON public.phone_agent_calls FOR ALL TO service_role
  USING (true) WITH CHECK (true);

DROP TRIGGER IF EXISTS trg_phone_agent_calls_updated_at ON public.phone_agent_calls;
