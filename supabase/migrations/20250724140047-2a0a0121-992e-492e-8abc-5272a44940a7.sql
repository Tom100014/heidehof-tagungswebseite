...
-- RLS Policies für die neuen Form-Typen
CREATE POLICY IF NOT EXISTS "Shop orders viewable by admins"
...
CREATE POLICY IF NOT EXISTS "Beauty appointments viewable by admins"
  ON unified_form_submissions FOR SELECT
...
CREATE POLICY IF NOT EXISTS "Bar Max orders viewable by admins"
  ON unified_form_submissions FOR SELECT
...
CREATE POLICY IF NOT EXISTS "Anyone can create shop orders"
  ON unified_form_submissions FOR INSERT
  WITH CHECK (form_type = 'shop_order');

CREATE POLICY IF NOT EXISTS "Anyone can create beauty appointments"
  ON unified_form_submissions FOR INSERT
  WITH CHECK (form_type = 'beauty_appointment');

CREATE POLICY IF NOT EXISTS "Anyone can create bar max orders"
  ON unified_form_submissions FOR INSERT
